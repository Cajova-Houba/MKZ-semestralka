...
import mkz.mkz_semestralka.core.error.BadMsgContentException;
import mkz.mkz_semestralka.core.error.BadMsgTypeReceived;
import mkz.mkz_semestralka.core.error.BadNickFormatException;
...
import mkz.mkz_semestralka.core.message.Message;
...
public class AbstractReceiver{
    ...
    protected MessageType receiveMessageType(DataInputStream inFromServer) throws IOException, ReceivingException {
        ...
        while (i < msgTypeLen) {
            ...
            if(msgStaus != 1) {
                logger.e("Error while receiving message type. Buffer position: "+i);
    ...
    private String receiveNick(DataInputStream input, char delimiter) throws IOException {
        ...
        while (((char)buffer[0]) != delimiter) {
            ...
            if (i >= Constrains.MAX_NICK_LENGTH) {
                return null;
            }
        ...
        if(i < Constrains.MIN_NICK_LENGTH) {
            // nick too short
            return null;
        }
    ...
    private AbstractReceivedMessage receiveInfMsg(DataInputStream inFromServer) throws IOException, ReceivingException {
        ...
        */
        ...
        if (received != 1) {
            logger.e("Error receiving INF message.");
            throw new EndOfStreamReached();
        }
        ...
        switch (firstChar) {
            ...
            case 'o':
                ...
                if(received != 1) {
                    logger.e("Error while receiving OK message");
                    throw new EndOfStreamReached();
                } else if (((char)buffer[1]) != 'K' && ((char)buffer[1]) != 'k') {
                    logger.e(String.format("Wrong message received: %c%c.", (char)buffer[0], (char)buffer[1]));
                    throw new BadMsgContentException();
                }

                logger.v("Received: OK");
            ...
            case 's':
                ...
                while(i < minLen) {
                    ...
                    if(received != 1) {
                        logger.e("Error while receiving START_GAME message.");
                        throw new EndOfStreamReached();
                    }

                    if(((char)buffer[i]) != c && ((char)buffer[i]) != Character.toLowerCase(c)) {
                        logger.e(String.format("Bad character %c on position %d while receiving START_GAME message.", (char)buffer[i], i));
                        throw new BadMsgContentException();
                    }
                ...
                if(firstNick == null || !Pattern.matches(Constrains.NICKNAME_REGEXP, firstNick)) {
                    logger.e("Error while receiving the 1st nick from START_GAME message");
                    throw new BadNickFormatException();
                }
                logger.v("1st nick received: "+firstNick);

                String secondNick = receiveNick(inFromServer, ';');
                if(secondNick == null || !Pattern.matches(Constrains.NICKNAME_REGEXP, firstNick)) {
                    logger.e("Error while receiving the 2nd nick from START_GAME message");
                    throw new BadNickFormatException();
                }
                logger.v("2nd nick received: "+secondNick   );
                logger.v("Received: START_GAME, p1="+firstNick+", p2="+secondNick);
            ...
            case 'e':
                ...
                while (i < minLen) {
                    ...
                    received = inFromServer.read(buffer,i,1);
                    if(received != 1) {
                        logger.e("Error while receiving END_GAME message.");
                        throw new EndOfStreamReached();
                    }

                    if(((char)buffer[i]) != c && ((char)buffer[i]) != Character.toLowerCase(c)) {
                        logger.e(String.format("Bad character %c on position %d while receiving END_GAME message.", (char)buffer[i], i));
                        throw new BadMsgContentException();
                    }
                    i++;
                }
            ...
            case 'a':
                ...
                while (i < minLen) {
                    ...
                    received = inFromServer.read(buffer,i,1);
                    if(received != 1) {
                        logger.e("Error while receiving ALIVE message.");
                        throw new EndOfStreamReached();
                    }

                    if(((char)buffer[i]) != c && ((char)buffer[i]) != Character.toLowerCase(c)) {
                        logger.e(String.format("Bad character %c on position %d while receiving ALIVE message.", (char)buffer[i], i));
                        throw new BadMsgContentException();
                    }
                    i++;
                }

                logger.v("Received ALIVE");
...
//                    logger.e("Error while receiving WAITING message.");
//                    throw new ReceivingException(Error.GENERAL_ERROR(ErrorMessages.RECEIVING_RESPONSE));
//                }
//
...
//                return new WaitingForPlayerReceivedMessage(nick);
    ...
    private ErrorReceivedMessage receiveErrMsg(DataInputStream inFromServer) throws IOException, ReceivingException {
        ...
        while (i < minLen) {
            ...
            if(received != 1) {
                logger.e("Error while receiving error message.");
                throw new EndOfStreamReached();
            }

            if(((char)buffer[i]) < '0' || ((char)buffer[i]) > '9') {
                logger.e(String.format("Bad character %c on position %d while receiving error message.", ((char)buffer[i]), i));
                throw new BadMsgContentException();
            }
        ...
        if (ec == ErrorCode.NO_ERROR) {
            logger.e("Error, wrong error code received.");
    ...
    private int[] receivePlayerStones(DataInputStream inFromServer, int player) throws IOException, ReceivingException {
        final int len = Constrains.MAX_NUMBER_OF_STONES;
        ...
        while(i < len*2) {
            received = inFromServer.read(buffer, i, 1);
            if(received != 1) {
                logger.e("Whole turn word of player "+player+" wasn't specified!");
                throw new BadMsgContentException();
            }

            if(((char)buffer[i]) < '0' || ((char)buffer[i]) > '9') {
                logger.e(String.format("Bad character %c on position %d while receiving turn word of player %d.", ((char)buffer[i]), i, player));
                throw new BadMsgContentException();
            }
    ...
    private AbstractReceivedMessage receiveCmdMsg(DataInputStream inFromServer) throws IOException, ReceivingException {
         ...
         */
        logger.d("Receiving CMD message.");
